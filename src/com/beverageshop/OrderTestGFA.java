...
public class OrderTestGFA {
	...
	public void testAddNewBeverage() throws NullPointerException {
		...
		orderOne.addNewBeverage("regular Coffee", Size.SMALL, false, false);
		assertTrue(((Beverage) orderOne.getBeverage(0)).getType().equals(Type.COFFEE));
		orderOne.addNewBeverage("Mohito", Size.SMALL);
		assertTrue(((Beverage) orderOne.getBeverage(1)).getType().equals(Type.ALCOHOL));
		orderOne.addNewBeverage("Detox", Size.MEDIUM, 1, false);
		assertTrue(((Beverage) orderOne.getBeverage(2)).getType().equals(Type.SMOOTHIE));
		assertTrue(orderOne.getTotalItems() == 3);

		orderTwo.addNewBeverage("Detox", Size.MEDIUM, 4, true);
		assertTrue(((Beverage) orderTwo.getBeverage(0)).getType().equals(Type.SMOOTHIE));
		orderTwo.addNewBeverage("Mohito", Size.SMALL);
		assertTrue(((Beverage) orderTwo.getBeverage(1)).getType().equals(Type.ALCOHOL));
		orderTwo.addNewBeverage("regular Coffee", Size.MEDIUM, true, false);
		assertTrue(((Beverage) orderTwo.getBeverage(2)).getType().equals(Type.COFFEE));
